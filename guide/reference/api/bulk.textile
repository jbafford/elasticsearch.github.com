...
The possible actions are @index@, @create@, @delete@ and since version @0.90.1@ also @update@.  @index@ and @create@ expect a source on the next line, and have the same semantics as the @op_type@ parameter to the standard index API (i.e., create will fail if a document with the same index and type exists already).  @delete@ does not expect a source on the following line, and has the same semantics as the standard delete API. @update@ expects that the partial doc, upsert and script and its options are specified on the next line.
